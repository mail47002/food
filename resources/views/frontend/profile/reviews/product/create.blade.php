...
@section('content')
    ...
    <div class="bg-yellow text-center">
        ...
        <h5 class="text-upper black margin-30">Відгук про повара</h5>
    ...
    <div class="container-half text-center">
        {{ Form::open(['route' => ['profile.reviews.product.store', $user->profile->slug], 'method' => 'post', 'class' => 'edit']) }}
        {{ Form::hidden('type', 'product') }}

        <p class="message" id="message">Заповніть виділені поля</p>

        <div class="form-group">
            <label>Виберіть страву, яку замовляли*</label>
            <select id="input-product_id" class="food wide" name="product_id">
        ...
        <div class="form-group">
            <label>Оцініть повара і страву*</label>
        ...
        $(document).on('submit', 'form', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response['success']) {
                        window.location = '{{ route('profile.reviews.product.success', $user->profile->slug) }}'
                    }
                },
                error: function(data) {
