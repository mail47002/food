...
@section('content')
    <div class="breadcrumbs">
        <div class="container">
            <ul class="list-inline">
                <li><a href="{{ route('profile.products.index') }}" class="link-blue back"><i class="fo fo-arrow-left fo-small"></i> Повернутися</a></li>
            </ul>
        </div>
    </div>
    <div class="bg-yellow">
    ...
    <div class="container-half text-center">
        {{ Form::open(['route' => ['profile.products.update', $product->id], 'method' => 'put', 'enctype' => 'multipart/form-data', 'class' => 'edit']) }}
            ...
            <div class="form-group">
                ...
                <div id="input-category" class="catgories clearfix">
                    @foreach ($categories as $category)
                        <div class="col-md-4">
                            {{ Form::checkbox('category[]', $category->id, in_array($category->id, $product->categories->pluck('category_id')->toArray()) ? true : false, ['id' => 'cat-' . $category->id]) }}
        ...
        {{ Form::close() }}
        <a href="{{ route('profile.products.index') }}" class="grey3">Відмінити</a>
    </div>
@stop
    ...
    <script type="text/javascript">
        $(document).on('submit', 'form', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data['success']) {
                        location = '{{ route('profile.products.success') }}';
                    }
                },
                error: function(data) {
                    var data = data.responseJSON;
